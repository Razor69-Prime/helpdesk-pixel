...
module.exports=function installLeaveApi(app,{db,requireAuth,logActivity}){
  ...
  const consumed=(requests,userId,year,excludeId)=>requests.filter(x=>String(x.applicant_user_id)===String(userId)&&String(x.id)!==String(excludeId)&&Number(String(x.start_date||'').slice(0,4))===Number(year)&&x.leave_type==='annual_leave'&&x.status==='approved').reduce((s,x)=>s+Number(x.duration_days||0),0);
  ...
  app.get('/api/leave/requests',requireAuth,async(req,res)=>{try{
    ...
    res.json({requests:rows,balances});
  ...
  app.post('/api/leave/requests',requireAuth,async(req,res)=>{try{
    const me=user(req),rows=await db.getLeaveRequests(),b=req.body||{};
    ...
    const row=await db.insertLeaveRequest({request_number:nextNo(rows),applicant_user_id:me.id,applicant_name:me.name,company:b.company||'Pixel Solusindo',division:b.division||'',job_title:b.job_title||'',start_date:b.start_date,end_date:b.end_date,duration_days:Number(b.duration_days||1),return_date:b.return_date||null,leave_type:b.leave_type,leave_type_other:b.leave_type_other||null,reason:b.reason,pic_user_id:b.pic_user_id||null,pic_name:b.pic_name||null,opening_balance:Number(b.opening_balance||0),remaining_balance:Number(b.remaining_balance||0),status:'draft',created_by:me.name,created_by_id:me.id});
  ...
  app.patch('/api/leave/requests/:id',requireAuth,async(req,res)=>{try{
    ...
    const allowed=['company','division','job_title','start_date','end_date','duration_days','return_date','leave_type','leave_type_other','reason','pic_user_id','pic_name'];
  ...
  app.post('/api/leave/requests/:id/decision',requireAuth,async(req,res)=>{try{
    ...
    if(decision==='approved'&&old.leave_type==='annual_leave'){
