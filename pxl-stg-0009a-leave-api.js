...
module.exports=function installLeaveApi(app,{db,requireAuth,logActivity}){
  ...
  const currentSnapshot=async()=>{
    ...
    const balances=stored.map(balance=>{
      const opening=Number(balance.opening_balance||0),used=consumed(requests,balance.user_id,balance.year);
    ...
    const enriched=requests.map(row=>{
      ...
      const balance=balances.find(x=>String(x.user_id)===String(row.applicant_user_id)&&Number(x.year)===year);
      return balance?{...row,opening_balance:Number(balance.opening_balance||0),remaining_balance:Number(balance.remaining_balance||0)}:row;
    });
