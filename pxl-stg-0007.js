...
/** PXL-STG-0007A–0007I — Kanban, jadwal, kapasitas, dan KPI teknisi. */
...
function serialize(t){const start=t.scheduled_date&&t.scheduled_start_time?`${String(t.scheduled_date).slice(0,10)}T${String(t.scheduled_start_time).slice(0,8)}`:null;return{id:t.id,wo_number:t.wo_number||null,project_name:t.project_name||t.project||'',customer_name:t.customer_name||t.customer||'',location:t.location||t.address||'',status:t.status||'assigned',technicians:technicians(t),scheduled_date:dateOnly(t.scheduled_date),scheduled_start_time:t.scheduled_start_time?String(t.scheduled_start_time).slice(0,5):null,estimated_duration_minutes:num(t.estimated_duration_minutes,60),schedule_priority:t.schedule_priority||'normal',schedule_order:num(t.schedule_order,0),schedule_notes:t.schedule_notes||'',is_late:Boolean(!closed(t)&&start&&new Date(start).getTime()<Date.now()),mr_status:t.material_request_status||t.mr_status||null};}
...
async function kpi(req,res){try{const db=getDb(),u=req.session?.user;if(!u||!VIEW.has(role(req)))return res.status(401).json({error:'Unauthorized'});let rows=await db.getTickets(null,true);if(['technician','teknisi'].includes(role(req)))rows=rows.filter(t=>assigned(t,u));const from=dateOnly(req.query.date_from),to=dateOnly(req.query.date_to);if(from)rows=rows.filter(t=>dateOnly(t.scheduled_date)>=from);if(to)rows=rows.filter(t=>dateOnly(t.scheduled_date)<=to);const map=new Map();for(const t of rows){for(const x of technicians(t)){const key=typeof x==='object'?String(x.id||x.name||''):String(x);if(!key)continue;const r=map.get(key)||{technician:key,total:0,done:0,running:0,waiting:0,late:0,minutes:0};r.total++;const s=norm(t.status);if(['done','completed','closed'].includes(s))r.done++;else if(['ongoing','in_progress','progress'].includes(s))r.running++;else if(['pending','waiting','menunggu'].includes(s))r.waiting++;if(serialize(t).is_late)r.late++;r.minutes+=num(t.estimated_duration_minutes,60);map.set(key,r);}}res.json({source:'PXL-STG-0007I',kpi:[...map.values()].map(r=>({...r,completion_rate:r.total?Math.round(r.done/r.total*100):0,capacity_hours:Math.round(r.minutes/6)/10}))});}catch(e){res.status(500).json({error:String(e.message||e)});}}
