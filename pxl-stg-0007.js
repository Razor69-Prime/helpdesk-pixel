...
function requestedTechnicians(body,current=[]){
  const source=body||{},values=[];
  const add=value=>{if(Array.isArray(value))value.forEach(add);else if(value&&typeof value==='object')values.push(value.id||value.user_id||value.technician_id||value.name||value.username);else if(value!=null&&String(value).trim())values.push(value);};
  ['technicians','technician','assigned_to','assigned_to2','technician_1','technician_2'].forEach(key=>{if(Object.prototype.hasOwnProperty.call(source,key))add(source[key]);});
  return values.length?[...new Map(values.filter(Boolean).map(value=>[norm(value),value])).values()].slice(0,2):current;
}
const approvedStatus=value=>norm(value)==='approved';
function dayBefore(value){const date=dateOnly(value);if(!date)return null;const d=new Date(`${date}T00:00:00Z`);d.setUTCDate(d.getUTCDate()-1);return d.toISOString().slice(0,10);}
function leaveEffectiveEnd(leave){const end=dateOnly(leave?.end_date),beforeReturn=dayBefore(leave?.return_date);return beforeReturn&&(!end||beforeReturn>end)?beforeReturn:end;}
function leaveCovers(leave,date){const end=leaveEffectiveEnd(leave);return Boolean(date&&approvedStatus(leave?.status)&&dateOnly(leave.start_date)&&end&&date>=dateOnly(leave.start_date)&&date<=end);}
async function approvedLeaves(from,to){
  const db=getDb();
  if(typeof db.getLeaveRequests!=='function')return[];
  const rows=await db.getLeaveRequests();
  return (Array.isArray(rows)?rows:[]).filter(x=>approvedStatus(x.status)&&dateOnly(x.start_date)&&leaveEffectiveEnd(x)&&(!from||leaveEffectiveEnd(x)>=from)&&(!to||dateOnly(x.start_date)<=to));
}
function leaveForTechnician(leaves,date,technician,aliases=[]){
  const keys=new Set([...techKeys(technician),...aliases.flatMap(techKeys)]);
...
function publicLeave(x){return{request_id:x.id||null,request_number:x.request_number||null,user_id:x.applicant_user_id||null,technician:x.applicant_name||null,start_date:dateOnly(x.start_date),end_date:leaveEffectiveEnd(x),requested_end_date:dateOnly(x.end_date),return_date:dateOnly(x.return_date),leave_type:x.leave_type||null};}
...
async function list(req,res){try{const db=getDb(),u=req.session?.user;if(!u||!VIEW.has(role(req)))return res.status(401).json({error:'Unauthorized'});let rows=await db.getTickets(null,true);if(['technician','teknisi'].includes(role(req)))rows=rows.filter(t=>assigned(t,u));const from=dateOnly(req.query.date_from||req.query.from||req.query.start_date),to=dateOnly(req.query.date_to||req.query.to||req.query.end_date);if(from)rows=rows.filter(t=>!sharedDate(t)||sharedDate(t)>=from);if(to)rows=rows.filter(t=>!sharedDate(t)||sharedDate(t)<=to);const techs=(await users()).filter(x=>['technician','teknisi'].includes(norm(x.role))&&x.is_active!==false),leaves=await approvedLeaves(from,to);const aliasMap=new Map();for(const tech of techs){const aliases=[tech.id,tech.user_id,tech.name,tech.full_name,tech.username].filter(Boolean);for(const key of aliases.map(norm))aliasMap.set(key,aliases);}const aliasesFor=value=>techKeys(value).flatMap(key=>aliasMap.get(key)||[]);const serialized=rows.map(serialize).map(t=>{const conflicts=technicians(t).map(x=>leaveForTechnician(leaves,t.scheduled_date,x,aliasesFor(x))).filter(Boolean);return{...t,leave_conflicts:[...new Map(conflicts.map(x=>[x.id,x])).values()].map(publicLeave)};});res.json({source:'PXL-STG-0009A11',timezone:WITA,can_edit:ADMIN.has(role(req)),tickets:serialized,technicians:techs.map(x=>({id:x.id,name:techName(x),username:x.username||null})),technician_leaves:leaves.map(publicLeave),leave_diagnostics:{date_from:from,date_to:to,approved_count:leaves.length}});}catch(e){res.status(500).json({error:String(e.message||e)});}}
async function update(req,res){try{const db=getDb(),u=req.session?.user;if(!u)return res.status(401).json({error:'Unauthorized'});if(!ADMIN.has(role(req)))return res.status(403).json({error:'Hanya Manager/Admin/Superadmin yang dapat mengubah jadwal.'});const rows=await db.getTickets(null,true),ticket=rows.find(t=>same(t.id,req.params.ticketId));if(!ticket)return res.status(404).json({error:'Work Order tidak ditemukan.'});const b=req.body||{},source=['kanban','calendar','ticket_list'].includes(norm(b.source))?norm(b.source):'kanban';const patch={scheduled_date:dateOnly(b.scheduled_date),scheduled_start_time:timeOnly(b.scheduled_start_time),estimated_duration_minutes:Math.max(1,Math.min(1440,num(b.estimated_duration_minutes,60))),schedule_priority:['low','normal','high','urgent'].includes(norm(b.schedule_priority))?norm(b.schedule_priority):'normal',schedule_order:num(b.schedule_order,0),schedule_notes:String(b.schedule_notes||'').slice(0,1000)||null,schedule_source:source,schedule_updated_at:new Date().toISOString(),schedule_updated_by:u.id||null};if(Array.isArray(b.technicians))patch.technicians=b.technicians.filter(Boolean).slice(0,2);
  const proposedTechs=requestedTechnicians(b,Array.isArray(patch.technicians)?patch.technicians:technicians(ticket));if(JSON.stringify(proposedTechs)!==JSON.stringify(technicians(ticket)))patch.technicians=proposedTechs;const scheduleChanged=patch.scheduled_date!==sharedDate(ticket)||JSON.stringify(proposedTechs)!==JSON.stringify(technicians(ticket));
