...
function servicePerm(req,permission){
  const u=req.session?.user||{},role=String(u.role||'').toLowerCase();
...
function serviceReminderLevel(row){
  ...
  const due=new Date(String(row.estimated_done_date).slice(0,10)+'T00:00:00'),today=new Date();today.setHours(0,0,0,0);
...
async function nextServiceNumber(){
  const year=new Date().getFullYear(),rows=await db.getServiceOrders();
...
app.get('/api/service-center/technicians',requireAuth,async(req,res)=>{
  try{if(!servicePerm(req,'service_create')&&!servicePerm(req,'service_assign'))return res.status(403).json({error:'Akses ditolak.'});const users=await db.getUsers();res.json((users||[]).filter(u=>String(u.role||'').toLowerCase()==='technician'&&u.is_active!==false).map(u=>({id:u.id,name:u.name})));}
  catch(e){res.status(500).json({error:e.message});}
});
...
app.get('/api/service-orders/:id',requireAuth,async(req,res)=>{
  try{const row=await db.getServiceOrder(req.params.id);if(!serviceCanOpen(req,row))return res.status(row?403:404).json({error:row?'Akses ditolak.':'Service tidak ditemukan.'});const[history,photos]=await Promise.all([db.getServiceHistory(row.id,false),db.getServicePhotos(row.id,false)]);res.json({...row,history,photos,reminder:serviceReminderLevel(row),contacts:{customer_care:SERVICE_CUSTOMER_CARE,technician:SERVICE_TECH_PHONE}});}
  catch(e){res.status(500).json({error:e.message});}
});
app.post('/api/service-orders',requireAuth,blockStagingDemoOnProduction,async(req,res)=>{
  try{
    ...
    const b=req.body||{};
    const now=new Date().toISOString();
    const row=await db.insertServiceOrder({service_number:await nextServiceNumber(),tracking_token:crypto.randomBytes(16).toString('hex'),customer_name:String(b.customer_name||'').trim()||null,customer_phone:String(b.customer_phone||'').trim()||null,customer_address:String(b.customer_address||'').trim()||null,device_type:String(b.device_type||'').trim()||null,brand:String(b.brand||'').trim()||null,model:String(b.model||'').trim()||null,serial_number:String(b.serial_number||'').trim()||null,complaint:String(b.complaint||'').trim()||null,initial_condition:String(b.initial_condition||'').trim()||null,accessories:Array.isArray(b.accessories)?b.accessories.map(x=>String(x).trim()).filter(Boolean):[],technician_user_id:b.technician_user_id||null,technician_name:String(b.technician_name||'').trim()||null,status:'received',estimated_done_date:b.estimated_done_date||null,received_at:now,created_by:req.session.user.name,updated_by:req.session.user.name});
...
app.patch('/api/service-orders/:id',requireAuth,async(req,res)=>{
  try{
    const current=await db.getServiceOrder(req.params.id);if(!serviceCanOpen(req,current))return res.status(current?403:404).json({error:current?'Akses ditolak.':'Service tidak ditemukan.'});
    if(!servicePerm(req,'service_update'))return res.status(403).json({error:'Tidak memiliki akses update service.'});
    const b=req.body||{},patch={updated_by:req.session.user.name};
    const allowed=servicePerm(req,'service_view_all')?['status','diagnosis','customer_update','estimated_done_date','technician_user_id','technician_name','estimated_cost','final_cost','customer_name','customer_phone','customer_address','device_type','brand','model','serial_number','initial_condition','complaint','accessories']:['status','diagnosis','customer_update'];
    ...
    if(patch.status!==undefined||b.timeline_note)await db.insertServiceHistory({service_id:current.id,status:patch.status||current.status,note:String(b.timeline_note||b.customer_update||'').trim()||null,customer_visible:b.customer_visible!==false,created_by:req.session.user.name});
...
app.post('/api/service-orders/:id/photos/signature',requireAuth,async(req,res)=>{
  try{
    ...
    const now=Date.now(),date=new Date(now).toISOString().slice(0,10).replace(/-/g,''),num=safeDocPart(row.service_number||row.id),public_id=`helpdesk-pixel/service-center/${num}/${num}-${date}-${now}-${Math.random().toString(36).slice(2,8)}`,timestamp=Math.floor(now/1000),params={public_id,timestamp};
