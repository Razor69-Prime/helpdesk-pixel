...
app.post('/api/service-orders',requireAuth,blockStagingDemoOnProduction,async(req,res)=>{
  try{
    ...
    const b=req.body||{};if(!b.customer_name||!b.customer_phone||!b.device_type||!b.complaint)return res.status(400).json({error:'Customer, nomor WhatsApp, jenis perangkat, dan keluhan wajib diisi.'});
    const now=new Date().toISOString();
    const row=await db.insertServiceOrder({service_number:await nextServiceNumber(),tracking_token:crypto.randomBytes(16).toString('hex'),customer_name:String(b.customer_name).trim(),customer_phone:String(b.customer_phone).trim(),device_type:String(b.device_type).trim(),brand:String(b.brand||'').trim()||null,model:String(b.model||'').trim()||null,serial_number:String(b.serial_number||'').trim()||null,complaint:String(b.complaint).trim(),initial_condition:String(b.initial_condition||'').trim()||null,accessories:Array.isArray(b.accessories)?b.accessories.map(x=>String(x).trim()).filter(Boolean):[],technician_user_id:b.technician_user_id||null,technician_name:String(b.technician_name||'').trim()||null,status:'received',estimated_done_date:b.estimated_done_date||null,received_at:now,created_by:req.session.user.name,updated_by:req.session.user.name});
...
app.patch('/api/service-orders/:id',requireAuth,async(req,res)=>{
  try{
    ...
    const allowed=servicePerm(req,'service_view_all')?['status','diagnosis','customer_update','estimated_done_date','technician_user_id','technician_name','estimated_cost','final_cost','initial_condition','complaint','accessories']:['status','diagnosis','customer_update'];
