...
(()=>{
 ...
 function loadShared(name,force=false){
   ...
   if(!force){const at=sharedLoadedAt.get(name)||0;if(at&&Date.now()-at<TTL)return Promise.resolve(true);if(sharedJobs.has(name))return sharedJobs.get(name);}
   const run=(async()=>{try{
     let data;
     if(name==='visits'){data=await api('GET','/sales-visits');if(typeof kunjunganData!=='undefined')kunjunganData=Array.isArray(data)?data:[];}
     else if(name==='mr'){data=await api('GET','/material-requests');if(typeof dashMRData!=='undefined')dashMRData=Array.isArray(data)?data:[];}
     else if(name==='pr'){data=await api('GET','/purchase-requests');const rows=Array.isArray(data)?data:[];if(typeof dashPRData!=='undefined')dashPRData=rows;if(typeof prData!=='undefined')prData=rows;}
     else if(name==='project'){data=await api('GET','/projects');if(typeof dashProjectData!=='undefined')dashProjectData=Array.isArray(data)?data:[];}
     else if(name==='invoice'){data=await api('GET','/invoices/standalone');if(typeof standaloneInvoices!=='undefined')standaloneInvoices=Array.isArray(data)?data:[];}
     else if(name==='targets'){data=await api('GET','/sales-targets');if(typeof salesTargets!=='undefined')salesTargets=Array.isArray(data)?data:[];}
     sharedLoadedAt.set(name,Date.now());renderShared(name);return data;
   }catch(e){console.warn('PXL-PROD-0022DB4 load '+name,e);return null;}finally{sharedJobs.delete(name);}})();
   sharedJobs.set(name,run);return run;
 }
...
// PXL-URG-0031G — force latest Purchase Request PDF renderer.
(function(){
  'use strict';
  document.querySelectorAll('script[data-pxl-pr-branding]').forEach(el=>el.remove());
  const s=document.createElement('script');
  s.dataset.pxlPrBranding='0031G';
  s.src='/pxl-urg-0031-pr-pdf-branding.js?v=PXL-URG-0031G';
