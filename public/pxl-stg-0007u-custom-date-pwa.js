/* PXL-STG-0007V — pisahkan Kanban dari Timeline/Custom Date. */
(function(){
  'use strict';
  ...
  const isTimelineActive=()=>!!$('[data-k7l-mode="timeline"]')?.classList.contains('active');
  ...
  function ensureSheet(){let sheet=$('#k7uSheet');if(sheet)return sheet;sheet=document.createElement('div');sheet.id='k7uSheet';sheet.hidden=true;sheet.innerHTML='<div class="k7u-sheet-card"><div class="k7u-sheet-head"><b>Detail WO</b><button type="button" class="k7u-close" aria-label="Tutup">×</button></div><div class="k7u-detail"></div></div>';document.body.appendChild(sheet);sheet.addEventListener('click',e=>{if(e.target===sheet||e.target.closest('.k7u-close'))sheet.hidden=true;});return sheet;}
  function showDetail(t){if(!t)return;const sheet=ensureSheet(),box=$('.k7u-detail',sheet);box.innerHTML=`<span>Nomor WO</span><strong>${esc(t.wo_number||'-')}</strong><span>Nama project</span><strong>${esc(t.project_name||'-')}</strong><span>Customer</span><strong>${esc(t.customer_name||'-')}</strong><span>Status</span><strong>${esc(t.status||'-')}</strong>`;sheet.hidden=false;}

  async function renderCustom(force=false){
    if(rendering||!isTimelineActive())return;
    const scale=$('#k7lScale');if(!scale||scale.value!=='custom')return;
    ...
    try{
      ...
      if(!isTimelineActive()||$('#k7lScale')?.value!=='custom')return;
    ...
    }catch(e){if(isTimelineActive()&&$('#k7lScale')?.value==='custom'){const body=$('#k7kBody');if(body)body.innerHTML=`<div class="k7u-empty">${esc(e.message)}</div>`;}}
    finally{rendering=false;}
  }

  function syncControls(){
    const scale=$('#k7lScale'),date=$('#k7lDate');if(!scale)return;
    const timeline=isTimelineActive();
    scale.style.display=timeline?'block':'none';
    if(date)date.style.display=timeline&&scale.value==='custom'?'block':'none';
  }
  ...
  function installCustomMode(){
    ...
    syncControls();
    if(!scale.dataset.k7uBound){scale.dataset.k7uBound='1';scale.addEventListener('change',()=>{syncControls();lastDate='';if(isTimelineActive()&&scale.value==='custom')setTimeout(()=>renderCustom(true),0);});}
    const date=$('#k7lDate');if(date&&!date.dataset.k7uBound){date.dataset.k7uBound='1';date.addEventListener('change',()=>{lastDate='';if(isTimelineActive()&&scale.value==='custom')renderCustom(true);});}
    $$('[data-k7l-mode]').forEach(btn=>{if(btn.dataset.k7uModeBound)return;btn.dataset.k7uModeBound='1';btn.addEventListener('click',()=>{lastDate='';setTimeout(()=>{syncControls();if(btn.dataset.k7lMode==='timeline'&&$('#k7lScale')?.value==='custom')renderCustom(true);},220);},true);});
    if(isTimelineActive()&&scale.value==='custom')renderCustom();
  }

  function decorateWeek(){if(!isTimelineActive()||!isPwa()||$('#k7lScale')?.value!=='week')return;const wrap=$('.k7l-wrap');if(wrap&&!wrap.previousElementSibling?.classList.contains('k7u-scroll-hint'))wrap.insertAdjacentHTML('beforebegin','<div class="k7u-scroll-hint">Geser ↔ untuk melihat tanggal lainnya</div>');$$('.k7l-chip').forEach(chip=>{if(chip.querySelector('.k7u-info'))return;const b=document.createElement('button');b.type='button';b.className='k7u-info';b.textContent='i';b.setAttribute('aria-label','Lihat detail WO');chip.appendChild(b);});}

  async function loadTicketForChip(chip){const id=String(chip.dataset.id||'');if(id&&ticketMap.has(id))return ticketMap.get(id);const range=$('#k7kRange')?.textContent||'',m=range.match(/(\d{1,2})\/(\d{1,2})\/(\d{4}).*?(\d{1,2})\/(\d{1,2})\/(\d{4})/);if(!m)return null;const from=`${m[3]}-${pad(m[2])}-${pad(m[1])}`,to=`${m[6]}-${pad(m[5])}-${pad(m[4])}`;const data=await api(`/api/technician-kanban?date_from=${from}&date_to=${to}`);(data.tickets||[]).forEach((t,i)=>{ticketMap.set(identity(t,i),t);if(t.id)ticketMap.set(String(t.id),t);});return ticketMap.get(id)||null;}

  document.addEventListener('click',async e=>{const card=e.target.closest('[data-k7u-key]');if(card&&isTimelineActive()){showDetail(ticketMap.get(card.dataset.k7uKey));return;}const info=e.target.closest('.k7u-info');if(info&&isTimelineActive()){e.preventDefault();e.stopImmediatePropagation();const chip=info.closest('.k7l-chip');showDetail(await loadTicketForChip(chip));}},true);

  function decorate(){installStyle();ensureSheet();installCustomMode();syncControls();decorateWeek();}
