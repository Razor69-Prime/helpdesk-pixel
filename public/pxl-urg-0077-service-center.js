...
(function(){
  ...
  async function openIntake(){
    await getTechs();
    modal('<div style="display:flex;justify-content:space-between;align-items:center"><div><b>Penerimaan Service</b><div style="font-size:11px;color:var(--muted)">Semua field bersifat opsional.</div></div><button class="btn sm" onclick="pxlServiceCenter.close()">✕</button></div><div class="form-row" style="margin-top:16px"><div class="form-group"><label>Nama Customer <span style="color:var(--muted)">(opsional)</span></label><input id="svc-name"></div><div class="form-group"><label>No. WhatsApp <span style="color:var(--muted)">(opsional)</span></label><input id="svc-phone" placeholder="08..."></div></div><div class="form-row"><div class="form-group"><label>Jenis Perangkat <span style="color:var(--muted)">(opsional)</span></label><select id="svc-type"><option value="">Pilih jika diperlukan...</option><option>Laptop</option><option>Komputer</option><option>Printer</option><option>Lainnya</option></select></div><div class="form-group"><label>Teknisi In Charge <span style="color:var(--muted)">(opsional)</span></label><select id="svc-tech"><option value="">Belum ditentukan</option>'+techs.map(t=>'<option value="'+esc(t.id)+'" data-name="'+esc(t.name)+'">'+esc(t.name)+'</option>').join('')+'</select></div></div><div class="form-row"><div class="form-group"><label>Brand <span style="color:var(--muted)">(opsional)</span></label><input id="svc-brand"></div><div class="form-group"><label>Model <span style="color:var(--muted)">(opsional)</span></label><input id="svc-model"></div></div><div class="form-row"><div class="form-group"><label>Serial Number <span style="color:var(--muted)">(opsional)</span></label><input id="svc-serial"></div><div class="form-group"><label>Estimasi Selesai <span style="color:var(--muted)">(opsional)</span></label><input id="svc-due" type="date"></div></div><div class="form-group"><label>Alamat <span style="color:var(--muted)">(opsional)</span></label><textarea id="svc-address" rows="2" placeholder="Input alamat secara manual"></textarea></div><div class="form-group"><label>Keluhan Customer <span style="color:var(--muted)">(opsional)</span></label><textarea id="svc-complaint" rows="3"></textarea></div><div class="form-group"><label>Kondisi Awal <span style="color:var(--muted)">(opsional)</span></label><textarea id="svc-condition" rows="2"></textarea></div><div class="form-group"><label>Kelengkapan <span style="color:var(--muted)">(opsional)</span></label><input id="svc-accessories" placeholder="Charger, tas, adaptor..."></div><div class="form-group"><label>Foto Penerimaan <span style="color:var(--muted)">(opsional)</span></label><input id="svc-files" type="file" accept="image/*" multiple><div style="font-size:11px;color:var(--muted)">Foto akan masuk ke Cloudinary dengan pola yang sama seperti foto WO.</div></div><div class="svc-actions" style="justify-content:flex-end"><button class="btn" onclick="pxlServiceCenter.close()">Batal</button><button class="btn primary" onclick="pxlServiceCenter.saveIntake()">Simpan & Buat Tanda Terima</button></div>');
  }
  ...
  async function saveIntake(){
    ...
    close();await refresh();await openDetail(created.id);
  }
  ...
  async function sendEReceipt(id,type='intake'){
    ...
    if(!waNum(r.customer_phone))return alert('Nomor WhatsApp customer belum diisi.');
  ...
  async function openDetail(id,printAfter){
    ...
    const intakeSigned=!!(r.intake_customer_signature&&r.intake_pixel_signature),handoverSigned=!!(r.handover_customer_signature&&r.handover_pixel_signature);
    const signActions='<button class="btn sm" onclick="pxlServiceCenter.openSignature(\''+r.id+'\',\'intake\')">✍️ '+(intakeSigned?'TTD Penerimaan ✓':'TTD Penerimaan')+'</button>'+(intakeSigned?'<button class="btn sm" onclick="pxlServiceCenter.openReceiptPdf(\''+r.id+'\',\'intake\')">📄 PDF Penerimaan</button><button class="btn sm" onclick="pxlServiceCenter.sendEReceipt(\''+r.id+'\',\'intake\')">📲 E-Tanda Terima</button>':'')+((has('service_update')&&['completed','ready_pickup','picked_up'].includes(r.status))?'<button class="btn sm" onclick="pxlServiceCenter.openSignature(\''+r.id+'\',\'handover\')">✍️ '+(handoverSigned?'TTD Pengembalian ✓':'TTD Pengembalian')+'</button>':'')+(handoverSigned?'<button class="btn sm" onclick="pxlServiceCenter.openReceiptPdf(\''+r.id+'\',\'handover\')">📄 PDF Pengembalian</button><button class="btn sm" onclick="pxlServiceCenter.sendEReceipt(\''+r.id+'\',\'handover\')">📲 E-Bukti Pengembalian</button>':'');

    const updateBlock=has('service_update')?'<div class="card" style="padding:14px"><b>Update Service</b><div class="form-row" style="margin-top:10px"><div class="form-group"><label>Status</label><select id="svc-up-status">'+Object.entries(STATUS).map(([k,v])=>'<option value="'+k+'" '+(r.status===k?'selected':'')+'>'+v+'</option>').join('')+'</select></div><div class="form-group"><label>Estimasi Selesai</label><input id="svc-up-due" type="date" value="'+esc(r.estimated_done_date||'')+'"></div></div><div class="form-group"><label>Update untuk Customer</label><textarea id="svc-up-note" rows="2">'+esc(r.customer_update||'')+'</textarea></div><div class="form-group"><label>Catatan Timeline</label><input id="svc-up-timeline" placeholder="Contoh: Diagnosa selesai, menunggu persetujuan"></div><button class="btn primary sm" onclick="pxlServiceCenter.saveUpdate(\''+r.id+'\')">Simpan Update</button></div>':'<div class="card" style="padding:14px;background:var(--surface2)"><b>Mode Read Only</b><div style="font-size:11px;color:var(--muted);margin-top:5px">Akun teknisi dapat melihat seluruh data Service Center. Perubahan status membutuhkan permission tambahan dari Manajemen Akun.</div></div>';
    const uploadBlock=has('service_photo')?'<div class="form-row" style="margin-top:12px"><div class="form-group"><label>Jenis Foto</label><select id="svc-photo-type"><option value="intake">Penerimaan</option><option value="progress">Progress</option><option value="completed">Selesai</option><option value="handover">Penyerahan</option></select></div><div class="form-group"><label>Upload Foto</label><input id="svc-photo-file" type="file" accept="image/*"></div></div><button class="btn sm" onclick="pxlServiceCenter.uploadDetailPhoto(\''+r.id+'\')">Upload Foto</button>':'';
    modal('<div style="display:flex;justify-content:space-between;gap:12px"><div><b>'+esc(r.service_number)+'</b><div style="font-size:11px;color:var(--muted)">'+esc(r.customer_name)+' · '+esc([r.device_type,r.brand,r.model].filter(Boolean).join(' '))+'</div></div><button class="btn sm" onclick="pxlServiceCenter.close()">✕</button></div><div class="svc-detail-grid" style="margin-top:14px"><div><div class="svc-kv"><span>Customer / WhatsApp</span><b>'+esc(r.customer_name||'-')+(r.customer_phone?' · <a target="_blank" href="https://wa.me/'+waNum(r.customer_phone)+'">'+esc(r.customer_phone)+'</a>':' · -')+'</b></div><div class="svc-kv"><span>Alamat</span><b>'+esc(r.customer_address||'-')+'</b></div><div class="svc-kv"><span>Keluhan</span><b>'+esc(r.complaint||'-')+'</b></div><div class="svc-kv"><span>Kondisi Awal</span><b>'+esc(r.initial_condition||'-')+'</b></div><div class="svc-kv"><span>Kelengkapan</span><b>'+esc((r.accessories||[]).join(', ')||'-')+'</b></div></div><div><div class="svc-kv"><span>Status</span><b>'+esc(STATUS[r.status]||r.status)+'</b></div><div class="svc-kv"><span>Estimasi</span><b>'+esc(d(r.estimated_done_date))+'</b></div><div class="svc-kv"><span>Teknisi</span><b>'+esc(r.technician_name||'-')+'</b></div><div class="svc-kv"><span>Biaya</span><b>'+esc(idr(r.final_cost||r.estimated_cost))+'</b></div></div></div><div class="svc-actions" style="margin:14px 0">'+signActions+'<button class="btn sm" onclick="pxlServiceCenter.sendWA(\''+r.id+'\')">📲 Update WA</button><button class="btn sm" onclick="navigator.clipboard.writeText(location.origin+\'/service/track/'+r.tracking_token+'\')">🔗 Copy Tracking</button></div>'+updateBlock+'<div class="card" style="padding:14px"><b>Foto Service</b><div class="svc-photo-grid" style="margin-top:10px">'+(photos||'<div style="font-size:11px;color:var(--muted)">Belum ada foto.</div>')+'</div>'+uploadBlock+'</div>');
  ...
  async function printReceipt(id){return openReceiptPdf(id,'intake')}
  async function sendWA(id){const r=await api('GET','/service-orders/'+id);if(!waNum(r.customer_phone))return alert('Nomor WhatsApp customer belum diisi.');window.open('https://wa.me/'+waNum(r.customer_phone)+'?text='+encodeURIComponent(waTemplate(r)),'_blank')}
