...
(function(){
  ...
  function taxSummary(items,so=null){
    const rows=Array.isArray(items)?items:[];
    const selected=rows.filter(x=>(x.ppn_applied===true||String(x.ppn_applied)==='true'||String(x.ppn_applied)==='1')&&n(x.ppn_rate)>0);
    const itemTax=selected.reduce((s,x)=>s+n(x.ppn_amount||0),0);
    const calculatedItemTax=selected.reduce((s,x)=>s+(n(x.ppn_amount)>0?n(x.ppn_amount):n(x.qty)*n(x.unit_price)*n(x.ppn_rate)/100),0);
    const base=rows.reduce((s,x)=>s+n(x.qty)*n(x.unit_price),0);
    const savedGrand=n(so?.quotation_total ?? so?.total_amount);
    const savedTax=savedGrand>base?savedGrand-base:0;
    const amount=Math.max(calculatedItemTax,itemTax,savedTax);
    const rates=[...new Set(selected.map(x=>n(x.ppn_rate)).filter(x=>x>0))];
    const fallbackRate=base>0&&amount>0?Math.round((amount/base)*10000)/100:0;
    return {
      amount,
      label:rates.length===1?`PPN ${rates[0]}%`:(rates.length?'PPN':(fallbackRate?`PPN ${fallbackRate}%`:'PPN'))
    };
  }

  function pdfUnitPrice(row){
    const applied=row?.ppn_applied===true||String(row?.ppn_applied)==='true'||String(row?.ppn_applied)==='1';
    const base=n(row?.unit_price);
    if(!applied) return base;
    const rate=n(row?.ppn_rate);
    return base + (base*rate/100);
  }
  function pdfLineTotal(row){
    return n(row?.qty)*pdfUnitPrice(row);
  }
