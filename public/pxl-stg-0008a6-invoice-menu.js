/* PXL-STG-0008A12 — buka Invoice V1 di iframe dan kirim token auth dari aplikasi utama. */
(function(){
  'use strict';
  ...
  function sendToken(frame){
    const value=token();
    if(!frame?.contentWindow || !value) return;
  ...
  function openInvoice(){
    ...
    wrap.innerHTML='<div style="height:48px;display:flex;align-items:center;justify-content:space-between;padding:0 14px;background:#fff;border-bottom:1px solid #ddd;font-family:Arial,sans-serif"><b>Invoice</b><button type="button" id="pxl-invoice-close" style="border:1px solid #d7cec6;background:#fff;border-radius:8px;padding:8px 12px;cursor:pointer">Tutup</button></div><iframe id="'+FRAME_ID+'" src="/invoice-v1.html?v=PXL-STG-0008A12" style="border:0;width:100%;flex:1;background:#f7f4f1" title="Invoice"></iframe>';
    ...
    frame.addEventListener('load',function(){
      sendToken(frame);
      setTimeout(()=>sendToken(frame),250);
      setTimeout(()=>sendToken(frame),1000);
    });
  }
  ...
  window.addEventListener('message',function(event){
    if(event.origin!==location.origin || event.data?.type!=='PXL_INVOICE_READY')return;
    sendToken(document.getElementById(FRAME_ID));
  });
  ...
  document.addEventListener('click',function(e){
    ...
    if(t&&String(t.textContent||'').trim()==='Invoice'&&t.closest('nav,.sidebar,.menu,.top-nav,#sidebar,.app-sidebar')){
      e.preventDefault();e.stopPropagation();openInvoice();
    }
  },true);
