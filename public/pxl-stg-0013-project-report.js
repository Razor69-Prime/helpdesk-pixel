/* PXL-STG-0013 — Fix role-aware Project Report/BOQ navigation. STAGING ONLY. */
(function(){
  'use strict';
  const REV='PXL-STG-0013';
  ...
  function renderReportDetail(){
    ...
    const items=r.items||[],wrap=$('#pr12-detail-wrap');const group=(cat,label)=>{const list=items.filter(i=>String(i.category).toLowerCase()===cat);return `<div class="pr12-section-title"><span>${label} <span class="pr12-muted">(${list.length})</span></span></div><div class="pr12-table-wrap"><table class="pr12-detail"><thead><tr><th>No</th><th>Nama</th><th>BOQ</th><th>Today</th><th>Total Done</th><th>Remain</th><th>Progress</th><th>Status</th><th>Today Achievement</th><th>History</th></tr></thead><tbody>${list.length?list.map((i,idx)=>reportItemRow(i,idx)).join(''):`<tr><td colspan="10" class="pr12-empty">BOQ ${label} belum diinput Manager.</td></tr>`}</tbody></table></div>`};wrap.innerHTML=group('material','Material')+group('jasa','Jasa');
    wrap.querySelectorAll('[data-pr12-ach]').forEach(b=>b.onclick=()=>addAchievement(b.dataset.pr12Ach));wrap.querySelectorAll('[data-pr12-hist]').forEach(b=>b.onclick=()=>toggleHistory(b.dataset.pr12Hist));
  }
  function reportItemRow(i,idx){const today=(i.achievements||[]).filter(a=>String(a.achievement_date).slice(0,10)===localDate()).reduce((s,a)=>s+n(a.achievement),0);return `<tr><td>${idx+1}</td><td><b>${h(i.item_name)}</b>${i.unit?`<div class="pr12-muted">Satuan: ${h(i.unit)}</div>`:''}${i.notes?`<div class="pr12-muted">${h(i.notes)}</div>`:''}<div id="pr12-hist-${h(i.id)}"></div></td><td class="pr12-num pr12-readonly">${fmt(i.boq_qty)}</td><td class="pr12-num">${fmt(today)}</td><td class="pr12-num">${fmt(i.total_done)}</td><td class="pr12-num">${fmt(i.remain)}</td><td>${bar(i.progress,i.category==='material'?'material':'jasa')}</td><td><span class="pr12-status ${statusClass(i.status)}">${h(i.status)}</span></td><td><div style="display:flex;gap:4px"><input class="pr12-today" type="number" min="0.01" step="0.01" max="${n(i.remain)}" id="pr12-ach-${h(i.id)}" placeholder="0" ${n(i.remain)<=0?'disabled':''}><button class="btn primary sm" data-pr12-ach="${h(i.id)}" ${n(i.remain)<=0?'disabled':''}>Simpan</button></div></td><td><button class="btn ghost sm" data-pr12-hist="${h(i.id)}">Lihat</button></td></tr>`}
  async function addAchievement(id){const input=$(`#pr12-ach-${CSS.escape(String(id))}`),value=Number(input?.value);if(!Number.isFinite(value)||value<=0){setError('#pr12-error','Today Achievement wajib lebih dari 0.');return}try{const u=await api('POST',`/project-report-items/${encodeURIComponent(id)}/achievements`,{achievement:value,achievement_date:localDate()});replace(u);renderReport();renderReportDetail();syncTrackerFromRows();setError('#pr12-error','')}catch(e){setError('#pr12-error',e.message||'Gagal menyimpan Today Achievement.')}}
