/* PXL-STG-0016 — Daily Project Report header metadata. STAGING ONLY. */
(function(){
  'use strict';
  const REV='PXL-STG-0016';
  ...
  function reportLastAchievement(r){
    const all=(r.items||[]).flatMap(i=>(i.achievements||[]).map(a=>({...a,item_id:i.id})));
    all.sort((a,b)=>String(b.created_at||b.achievement_date||'').localeCompare(String(a.created_at||a.achievement_date||'')));
    return all[0]||null;
  }
  function reportProjectId(r){
    const so=r.so_number||r.no_so||r.sales_order_number||r.sales_order_no||r.sales_order?.so_number||'';
    const wo=r.wo_number||r.no_wo||r.work_order_number||r.work_order_no||r.work_order?.no_wo||'';
    if(so&&wo&&String(so)!==String(wo)) return `${so} / ${wo}`;
    return String(so||wo||r.project_code||r.project_no||'-');
  }
  function indoReportDate(date){
    if(!date)return '-';
    const d=new Date(`${String(date).slice(0,10)}T12:00:00+08:00`);
    const tanggal=new Intl.DateTimeFormat('id-ID',{timeZone:'Asia/Makassar',day:'numeric',month:'long',year:'numeric'}).format(d);
    const hari=new Intl.DateTimeFormat('id-ID',{timeZone:'Asia/Makassar',weekday:'long'}).format(d);
    return `${tanggal} / ${hari}`;
  }
  function itemToday(i,date){return (i.achievements||[]).filter(a=>String(a.achievement_date).slice(0,10)===date).reduce((s,a)=>s+n(a.achievement),0)}
  function dailyReportData(){
    const r=current();if(!r)return null;
    const last=reportLastAchievement(r);
    const date=String(last?.achievement_date||localDate()).slice(0,10);
    const pic=String(last?.created_by||currentUser()?.name||currentUser()?.nama||'-');
    const groups=['material','jasa'].map(category=>({category,label:category==='material'?'MATERIAL':'JASA',items:(r.items||[]).filter(i=>String(i.category).toLowerCase()===category).map(i=>({...i,today_report:itemToday(i,date)}))}));
    return {r,date,pic,team:'-',projectId:reportProjectId(r),groups};
  }
  function dailyReportText(){
    const d=dailyReportData();if(!d)return '';
    const {r,date,pic,team,projectId,groups}=d;
    const year=String(date||localDate()).slice(0,4);
    const lines=[`DAILY REPORT PROJECT PIXEL ${year}`,'',`Date/Days    : ${indoReportDate(date)}`,`Project Name : ${r.nama_project||'-'}`,`Project ID   : ${projectId}`,`PIC          : ${pic}`,`Team         : ${team}`,''];
    groups.forEach(g=>{lines.push(g.label);if(!g.items.length)lines.push('- Belum ada BOQ');g.items.forEach((i,idx)=>{lines.push(`${idx+1}. ${i.item_name||'-'}`);lines.push(`   BOQ: ${fmt(i.boq_qty)} | Today: ${fmt(i.today_report)} | Total Done: ${fmt(i.total_done)} | Remain: ${fmt(i.remain)} | ${i.status||'-'}`)});lines.push('')});
    lines.push('SUMMARY',`Material: ${pct(r.material_summary?.progress)}`,`Jasa: ${pct(r.jasa_summary?.progress)}`,`Overall: ${pct(r.progress)}`);
    return lines.join('\n');
  }
  ...
  function downloadDailyReportPdf(){
    const d=dailyReportData();if(!d)return;
    if(!window.jspdf?.jsPDF){alert('Library PDF belum siap. Refresh halaman lalu coba kembali.');return}
    const {jsPDF}=window.jspdf;const doc=new jsPDF({orientation:'portrait',unit:'mm',format:'a4'});
    const {r,date,pic,team,projectId,groups}=d;const year=String(date||localDate()).slice(0,4);
    doc.setFontSize(15);doc.text(`DAILY REPORT PROJECT PIXEL ${year}`,14,16);
    doc.setFontSize(10);doc.text(`Date/Days: ${indoReportDate(date)}`,14,23);doc.text(`Project Name: ${r.nama_project||'-'}`,14,28);doc.text(`Project ID: ${projectId}`,14,33);doc.text(`PIC: ${pic}`,14,38);doc.text(`Team: ${team}`,14,43);
    let y=50;
    groups.forEach(g=>{doc.setFontSize(11);doc.text(g.label,14,y);y+=3;const body=g.items.map((i,idx)=>[idx+1,i.item_name||'-',fmt(i.boq_qty),fmt(i.today_report),fmt(i.total_done),fmt(i.remain),i.status||'-']);doc.autoTable({startY:y,head:[['No','Nama','BOQ','Today','Done','Remain','Status']],body:body.length?body:[['','Belum ada BOQ','','','','','']],styles:{fontSize:7,cellPadding:1.5},headStyles:{fontStyle:'bold'},columnStyles:{0:{cellWidth:8},1:{cellWidth:78},2:{cellWidth:16},3:{cellWidth:16},4:{cellWidth:16},5:{cellWidth:16},6:{cellWidth:27}},margin:{left:14,right:14}});y=doc.lastAutoTable.finalY+7;if(y>250){doc.addPage();y=16}});
    doc.setFontSize(11);doc.text('SUMMARY',14,y);doc.setFontSize(10);doc.text(`Material: ${pct(r.material_summary?.progress)}   Jasa: ${pct(r.jasa_summary?.progress)}   Overall: ${pct(r.progress)}`,14,y+6);
    const safe=String(r.nama_project||'project').replace(/[^\w\-]+/g,'_').slice(0,50);doc.save(`Project_Daily_Report_${safe}_${date}.pdf`);
  }
